<script>
    import { selectedCategoryId, isCountryLoaded ,selectedProgramId} from '$lib/stores.js';
    ...
    const fetchProgramData = async () => {
      ...
      try {
        ...
        isCountryLoaded.set(true); // Set loading state to true after fetching data
      }
    };
      ...
      {#each programs as program}
        <li key={program.program_id}>
          <Input
            ...
            type="button"
            className={`inline-flex items-center justify-center w-full p-2 text-black border-2 border-black rounded-lg cursor-pointer text-lg font-semibold ${selectedId === program.program_id ? 'bg-green-600 text-white' : ''}`}
